export function setupEnableTokenDynamicRing() {
  Hooks.on("preCreateToken", async (tokenDocument) => {
    ...
    if (game.release.generation < 12) {
      ...
      if (ringColor !== "None" && !foundry.utils.getProperty(tokenDocument, "flags.tokenRing.colors.ring")) {
        tokenDocument.updateSource({"flags.dnd5e.tokenRing.colors.ring": `${getHexColor(ringColor)}`});
      }
